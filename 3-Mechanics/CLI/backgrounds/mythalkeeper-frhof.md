---
obsidianUIMode: preview
cssclasses:
- json5e-background
tags:
- ttrpg-cli/background
- ttrpg-cli/compendium/src/5e/frhof
aliases:
- "Mythalkeeper"
---
# Mythalkeeper
*Source: Forgotten Realms: Heroes of Faerûn p. 34*  
![](file://D:\ProgramData\CLI\5etools-img/backgrounds/FRHoF/Mythalkeeper.webp#right)

- **Ability Scores.** Intelligence, Wisdom, Charisma  
- **Feat.** [Crafter](/3-Mechanics/CLI/feats/crafter-xphb.md)  
- **Skill Proficiencies.** [Arcana](/3-Mechanics/CLI/skills.md#Arcana) and [History](/3-Mechanics/CLI/skills.md#History)  
- **Tool Proficiencies.** [Jeweler's Tools](/3-Mechanics/CLI/items/jewelers-tools-xphb.md)  
- **Equipment.** Choose A or B: (A) [Quarterstaff](/3-Mechanics/CLI/items/quarterstaff-xphb.md), [Jeweler's Tools](/3-Mechanics/CLI/items/jewelers-tools-xphb.md), [Perfume](/3-Mechanics/CLI/items/perfume-xphb.md), [Pouch](/3-Mechanics/CLI/items/pouch-xphb.md), [Robe](/3-Mechanics/CLI/items/robe-xphb.md), [Shovel](/3-Mechanics/CLI/items/shovel-xphb.md), [String](/3-Mechanics/CLI/items/string-xphb.md), [Waterskin](/3-Mechanics/CLI/items/waterskin-xphb.md), 16 GP; or (B) 50 GP  

Mythals are sources of great magical power that can alter the Weave or even the very nature of reality. Most were constructed in antiquity, and many have since been damaged or gone dormant. As a mythalkeeper from the Dalelands, your first experience with a mythal was likely in the ruins of Myth Drannor. You roam Faerûn in search of other ruined places of power, hoping to learn more about the history and powers of mythals—or even restore a malfunctioning one.